...
class Robot:
    ...
    def init_robot(self):
        ...
        self.integral_array = list()
        self.derivative_array = list()
        self.x_axis = list()
        self.index = count()
    ...
    def line_movement_PID(self, cx, drive_speed, half_frame, integral, derivative, last_error):  #PID line follower
        ...
        last_error = error
        print("Left: %s , Right: %s , cx = %s" %(self.left.dutycycle, self.right.dutycycle, cx))
        self.x_axis.append(next(self.index))
        self.error_array.append(last_error)
        self.integral_array.append(integral)
        self.derivative_array.append(derivative)

        plt.plot(self.x_axis, self.error_array, color = "lightsalmon")
        plt.plot(self.x_axis, self.integral_array, color = "lightgreen")
        plt.plot(self.x_axis, self.derivative_array, color = "skyblue")
        plt.plot(self.x_axis, cx, color = "gray")
        plt.legend(["error", "integral", "derivative", "cx"], loc="upper right")
